            ...
            foreach my $storage_type ('StandardStorage', 'StandardIAStorage', 'ReducedRedundancyStorage') {
                my $entry = { label => lc($metric) . '-' . lc($storage_type) . '-' . lc($statistic), set => {
                                    ...
                                    perfdatas => [
                                        ...
                                        template => '%d', unit => 'B', label_extra_instance => 1, instance_use => 'display_absolute' },
                                    ],
                                }
                            };
        ...
        foreach my $storage_type (@{$self->{aws_storage_type}}) {
            ...
            foreach my $metric (@{$self->{aws_metrics}}) {
                foreach my $statistic (@{$self->{aws_statistics}}) {
                    ...
                    $self->{metric}->{$instance . "_" . $storage_type . "_" . lc($statistic)}->{$metric . "_" . $storage_type . "_" . lc($statistic)} = $metric_results{$instance}->{$metric}->{lc($statistic)};
                }
            }
        }
    }
...
Filter metrics (Can be: 'BucketSizeBytes') 
(Can be a regexp).
