...
package cloud::aws::ec2::mode::asginstances;
...
sub set_counters {
    ...
    $self->{maps_counters_type} = [
        { name => 'aws_asg', type => 1, cb_prefix_output => 'prefix_awsasg_output', message_multiple => 'All Autoscaling group are ok' },
...
sub manage_selection {
    ...
    foreach (@{$result}) {
        ...
        if (defined($self->{option_results}->{filter_asg}) && $self->{option_results}->{filter_asg} ne '' &&
            $_->{AutoScalingGroupName} !~ /$self->{option_results}->{filter_asg}/) {
            $self->{output}->output_add(long_msg => "skipping asg '" . $_->{AutoScalingGroupName} . "': no matching filter.", debug => 1);
            next;
        }
        ...
        $self->{aws_asg}->{$_->{AutoScalingGroupName}} = { display => $_->{AutoScalingGroupName},
                                                           min_size => $_->{MinSize},
                                                           max_size => $_->{MaxSize},
...
Check Autoscaling groups related instances (number of instances within, state of each instances)

perl centreon_plugins.pl --plugin=cloud::aws::ec2::plugin --custommode='paws' --mode=asg-instances --region=eu-west-1 --aws-secret-key='xxxx' --aws-access-key='xxxxxxxxxxx' --critical-instances='%{health} =~ /Healthy/ && %{lifecycle} !~ /InService/' --warning-count 10
