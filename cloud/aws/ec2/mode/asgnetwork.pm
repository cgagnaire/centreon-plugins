...
package cloud::aws::ec2::mode::asgnetwork;
...
sub custom_traffic_perfdata {
    ...
    $self->{output}->perfdata_add(label => lc($self->{result_values}->{metric}) . "_" . lc($self->{result_values}->{display}) ,
...
Check EC2 Auto Scaling Group network metrics.

Example: 
perl centreon_plugins.pl --plugin=cloud::aws::plugin --custommode=paws --mode=ec2-asg-net --region='eu-west-1' --asg-name='centreon-middleware' --filter-metric='Packets' --statistic='sum' ---critical-networkpacketsout-sum='10' --verbose
