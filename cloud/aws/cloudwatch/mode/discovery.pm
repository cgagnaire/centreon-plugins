...
sub run {
    ...
    my $vpcs = $options{custom}->discovery(region => $self->{option_results}->{region}, service => 'ec2', command => 'describe-vpcs');
    my $instances = $options{custom}->discovery(region => $self->{option_results}->{region}, service => 'ec2', command => 'describe-instances');
    my $db_instances = $options{custom}->discovery(region => $self->{option_results}->{region}, service => 'rds', command => 'describe-db-instances');
    my $load_balancers = $options{custom}->discovery(region => $self->{option_results}->{region}, service => 'elb', command => 'describe-load-balancers');
    my $vpns = $options{custom}->discovery(region => $self->{option_results}->{region}, service => 'ec2', command => 'describe-vpn-connections');
    ...
    foreach my $reservation (@{$instances->{Reservations}}) {
        foreach my $instance (@{$reservation->{Instances}}) {
            ...
            $ec2{vpc_name} = $vpcs{$instance->{VpcId}};
    ...
    foreach my $db_instance (@{$db_instances->{DBInstances}}) {
        ...
        $rds{vpc_name} = $vpcs{$db_instance->{DBSubnetGroup}->{VpcId}};
        push @disco_data, \%rds;
    }
    ...
    foreach my $load_balancer (@{$load_balancers->{LoadBalancerDescriptions}}) {
        ...
        $elb{vpc_name} = $vpcs{$load_balancer->{VPCId}};
        push @disco_data, \%elb;
    }
