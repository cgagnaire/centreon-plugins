...
sub set_counters {
    ...
    $self->{maps_counters_type} = [
        { name => 'elb_perf', type => 1, cb_prefix_output => 'prefix_elb_output', message_multiple => "Hosts behind the ELB are OK", skipped_code => { -10 => 1 } },
    ];
    ...
    foreach my $statistic (('maximum', 'average', 'sum')) {
	    ...
	    next if ($metric_name eq 'RequestCount' && $statistic ne 'sum' || $metric_name eq 'Latency' && $statistic !~ /average|maximum/);
            my $entry = { label => lc($metric_name) . '-' . lc($statistic), set => {
                                ...
                                output_template => ($metric_name eq 'Latency') ? $metric_name . ' ' . $statistic . ': %.2f sec' : $metric_name . ' ' . $statistic . ': %d requests',
                                perfdatas => [
...
sub manage_selection {
    ...
    $self->{aws_metrics} = ['RequestCount', 'Latency'];
    ...
    foreach my $host_stat (keys %{$metric_results}) {
        foreach my $stat (('average', 'sum', 'maximum')) {
            ...
            $self->{elb_perf}->{$self->{option_results}->{elb_name}}->{display} = $self->{option_results}->{elb_name};
            $self->{elb_perf}->{$self->{option_results}->{elb_name}}->{$host_stat . '_' . $stat} = $metric_results->{$host_stat}->{$stat};
	    $self->{elb_perf}->{$self->{option_results}->{elb_name}}->{stat} = $stat;
	}
    }
